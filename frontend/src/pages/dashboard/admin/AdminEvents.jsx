  ...
  return (
    <div style={{ background: 'var(--white)', border: '1px solid var(--g100)', borderRadius: 14, overflow: 'hidden' }}>
      {ev.banner ? (
        ...
        <div style={{ width: '100%', height: 160, background: 'linear-gradient(135deg,#0891b2,#0e7490)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ background: 'linear-gradient(160deg,#111118 0%,#1a1200 100%)', padding: '36px 0 32px' }}>
        <div className="container">
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
            <div>
              <h1 style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(22px,3vw,30px)', fontWeight: 700, color: '#fff', margin: '0 0 6px' }}>AUSI-Wide Events</h1>
              <p style={{ fontSize: 13.5, color: 'rgba(255,255,255,.4)', margin: 0 }}>Events published here are visible to all AUSI members across India.</p>
            </div>
      ...
      <div className="container" style={{ paddingTop: 28, paddingBottom: 64 }}>
        ...
        {creating && (
          <div style={{ background: 'var(--white)', border: '1px solid var(--g100)', borderRadius: 14, padding: '24px', marginBottom: 28 }}>
            <h2 style={{ margin: '0 0 4px', fontSize: 16, fontWeight: 700, color: 'var(--ink)' }}>New AUSI-Wide Event</h2>
            <p style={{ fontSize: 13, color: 'var(--g500)', margin: '0 0 20px' }}>Visible to all students across all universities.</p>
            ...
            <div style={{ display: 'grid', gap: 16 }}>
              <Field label="Event Title *">
                <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
                  placeholder="e.g. Uganda Independence Day Gala 2026" style={inp} />
              </Field>
              ...
              <Field label="Location">
                <input value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))}
                  placeholder="e.g. GITAM University, Visakhapatnam" style={inp} />
              </Field>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <button onClick={handleSave} disabled={saving}
                  style={{ padding: '10px 24px', background: '#0891b2', color: '#fff', border: 'none',
        ...
        {events.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '72px 0', color: 'var(--g400)', fontSize: 14 }}>
            No AUSI-wide events yet. Click "+ New Event" to create one.
          </div>
        ) : (
          <>
